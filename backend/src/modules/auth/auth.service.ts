...
export class AuthService {
  ...
  async checkPhone(
    phone: string
  ): Promise<{ isMember: boolean; hasPassword: boolean }> {
    ...
    return {
      isMember: true,
      hasPassword: !!(user as { passwordHash?: string }).passwordHash,
    };
  }
  ...
  async requestOtp(
    ...
    const res = await fetch(
      `${FIREBASE_PHONE_BASE}:sendVerificationCode?key=${apiKey}`,
      {
        method: 'POST',
    ...
    if (!res.ok) {
      ...
      throw new InternalServerErrorException(
        err?.error?.message ?? 'Failed to send SMS'
      );
    }
  ...
  async setPassword(
    ...
    if (user.passwordHash) {
      throw new ConflictException(
        'Password already set — use account recovery to change it'
      );
    }
  ...
  async loginWithPassword(phone: string, password: string): Promise<TokenPair> {
    ...
    if (!user.passwordHash) {
      throw new UnauthorizedException(
        'No password set — please log in with SMS OTP first'
      );
    }

    const valid = await bcrypt.compare(password, user.passwordHash);
    if (!valid) {
      this.log('LOGIN_FAILED', 'warn', {
        phone,
  ...
  async purgeExpiredRefreshTokens(): Promise<number> {
    const res = await this.refreshTokens.deleteMany({
  ...
  private async issueTokenPair(
    ...
    const access_token = this.jwt.sign(
      this.buildPayload(userId, permissions, googleEmail)
    );
  ...
  private async exchangeOtp(
    ...
    const res = await fetch(
      `${FIREBASE_PHONE_BASE}:signInWithPhoneNumber?key=${apiKey}`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    return e164.startsWith(COUNTRY_CODE)
      ? e164.slice(COUNTRY_CODE.length)
      : e164;
  }
  ...
  private async findUserByPhone(localPhone: string): Promise<UserRow> {
    const user = (await this.users
      ...
      .lean()) as UserRow | null;
    if (!user)
      throw new NotFoundException(`No member found for phone ${localPhone}`);
    return user;
  }
