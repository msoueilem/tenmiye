...
export class RolesService {
  ...
  async findAll() {
    const snap = await this.firebase.db.collection(COLLECTION).orderBy('createdAt').get();
    return snap.docs.map((d) => ({ id: d.id, ...serializeDoc(d.data()) }));
  }

  async findOne(id: string) {
    const doc = await this.firebase.db.collection(COLLECTION).doc(id).get();
    if (!doc.exists) throw new NotFoundException(`Role ${id} not found`);
  ...
  async create(dto: CreateRoleDto, createdBy: string) {
    ...
    if (!dup.empty) throw new ConflictException(`Role slug '${dto.slug}' already exists`);
