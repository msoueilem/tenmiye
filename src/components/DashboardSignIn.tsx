...
export function DashboardSignIn() {
  ...
  useEffect(() => {
    // Redirect already-authenticated admins straight to dashboard
    if (tokenStore.getAccess('admin')) {
      router.replace('/admin');
      return;
    }
    // Surface any error forwarded from the callback page
    const err = searchParams.get('error');
    if (err === 'auth_failed') setError('فشل تسجيل الدخول. يرجى المحاولة مرة أخرى.');
  }, [router, searchParams]);
            ...
            <div className="dark:border-dashboard-primary/10 relative mx-auto w-full max-w-md overflow-hidden rounded-2xl border border-slate-100 bg-white p-8 shadow-xl md:p-10 dark:bg-[#1a2e1d]">
              ...
              <div className="space-y-6">
                ...
                <button
                  ...
                  className="group flex h-12 w-full items-center justify-center gap-3 rounded-lg border border-slate-300 bg-white px-6 font-medium text-slate-700 shadow-sm transition-all hover:border-slate-400 hover:bg-slate-50"
                >
                  <svg
                    className="h-5 w-5"
                    viewBox="0 0 24 24"
